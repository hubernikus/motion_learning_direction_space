...
class DirectionalGMM(LearnerVisualizer, Learner):
    ...
    def _predict(self, X, input_output_normalization=True,
        ...
        if convergence_attractor:
            ...
            beta[:,dist_attr!=0] = (beta[:, dist_attr!=0] / np.tile(
                np.linalg.norm(beta[:,dist_attr!=0], axis=0), (self.n_gaussians+1, 1)))
    ...
    def get_mean_yx(self, X, stretch_input_values=False):
        ...
        mu_yx = np.zeros((dim-dim_in, n_samples, self.n_gaussians))
        mu_yx_test = np.zeros((dim-dim_in, n_samples, self.n_gaussians))
