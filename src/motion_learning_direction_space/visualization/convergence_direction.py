...
def test_convergence_direction_multihull(obstacle_list, it_obs, n_resolution=30, x_lim=None,
    ...
    attractor_is_inside = obstacle_list[it_obs].get_gamma(obstacle_list.pos_attractor,
                                                          in_global_frame=True) > 1
    for ix in range(n_resolution):
        for iy in range(n_resolution):
            positions[:, ix, iy] = [x_vals[ix], y_vals[iy]]
            ind_no_collision[ix, iy] = (obstacle_list[it_obs].get_gamma(
                positions[:, ix, iy], in_global_frame=True) >= 1)
    ...
    ax.plot(obstacle_list.pos_attractor[0], obstacle_list.pos_attractor[1], 'k*', zorder=5)
